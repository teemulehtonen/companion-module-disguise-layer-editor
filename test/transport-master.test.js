...
function instance(){
 ...
 ],lastError:'',log(){},connectionStatus(){},publish(){this.publishes=(this.publishes||0)+1},saveConfig(config){this.saved={...config}}})
 return item
}

test('master selection clamps at the transport list ends and cancels an unsent old value',()=>{
...
test('master transport page selects an exact slot and ignores empty slots',()=>{
 const item=instance();item.masterLevelPending={uid:'1',percent:25}
...
test('published motor target uses the lower level when brightness and volume differ',()=>{
 ...
 assert.equal(values.master_transport_position,'1/2')
