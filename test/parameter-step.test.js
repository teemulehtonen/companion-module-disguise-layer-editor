...
const cases=[
 [{min:-180,max:180},[36,3.6,.36]],
 [{min:0,max:1},[.1,.01,.001]],
 [{min:0,max:100},[10,1,.1]],
 [{min:100,max:200},[10,1,.1]],
 [{min:-1,max:1},[.2,.02,.002]],
 [{min:0,max:.01},[.001,.0001,.00001]],
 [{min:0,max:1.000000047},[.1,.01,.001]],
 [{},[.1,.01,.001]],
 [{min:null,max:180},[.1,.01,.001]],
 [{min:180,max:-180},[.1,.01,.001]],
 [{min:1,max:1},[.1,.01,.001]],
 [{min:0,max:100,integer:true},[10,1,1]],
...
test('default parameter steps span full signed ranges and preserve integer resolution',()=>{
 ...
 assert.equal(parameterStep({min:-Infinity,max:Infinity},{}),.1)
 assert.equal(parameterStep({min:NaN,max:100},{}),.1)
...
test('editor uses scaled steps for selected keys and constants and clamps both ends',async()=>{
 for(const sequenced of [true,false]) {
  for(let i=0;i<3;i++) {
   ...
   assert.equal(editor.value,[36,3.6,.36][i])
...
test('small-range values remain visible at every precision and enums remain discrete',async()=>{
 assert.equal(parameterDecimals({min:0,max:.01},'ultra'),5)
 const client=new DemoClient(),f=client.data.layers[0].fields[0]
 ...
 await editor.adjustLiveValue(1);assert.equal(editor.valueLabel,'0.00001')
