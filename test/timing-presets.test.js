...
test('adaptive step slots expose correct time, beat, layer and key choices without releasing selection',async()=>{
 ...
 assert.deepEqual(e.timeStepChoices.map(c=>c.label),['1 FRAME','0.5 SEC','1 SEC','2 SEC','5 SEC','10 SEC','30 SEC','1 MIN','2 MIN','5 MIN'])
 for (const [slot,seconds] of [[1,0.5],[6,30],[8,120],[9,300]]) { e.setTimeStep(slot); assert.equal(e.timeStepAmount,seconds); assert.equal(e.timeStepLabel,e.timeStepChoices[slot].label) }
