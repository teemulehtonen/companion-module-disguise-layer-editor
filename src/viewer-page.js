...
function browserMain(applyEditPatch, discreteSegments, selectionScroll, timecode, duplicateMarkerKeys) {
  ...
  document.addEventListener('keydown', event => {
    ...
    if (event.key.toLowerCase()==='l' && !event.ctrlKey && !event.metaKey && !event.altKey) {
      event.preventDefault()
      if(!event.repeat) (event.shiftKey ? $('fitLayer') : linkTimeButton).click()
      return
    }
