...
function browserMain() {
  ...
  function draw() {
    ...
    for (const [title, kind] of [
      ...
      for (const item of entries) {
        ...
        node.setAttribute('role', 'button')
        node.tabIndex = 0
        node.setAttribute('aria-label', 'Go to ' + title + ' ' + node.textContent)
        node.onclick = async event => {
          event.stopPropagation()
          try {
            const response = await fetch('/api/seek', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json', 'X-Viewer-Token': state.selectionToken },
              body: JSON.stringify({ trackUid: state.trackUid, time: item.time }),
              signal: AbortSignal.timeout(10000),
            })
            const result = await response.json()
            $('selectionMessage').textContent = result.ok ? '' : result.reason || 'Seek unavailable'
  ...
  function updatePlayhead() {
    ...
    for (const head of sheet.querySelectorAll('.header-playhead')) {
      head.style.left = x(state.time) + '%'
      head.hidden = x(state.time) < 0 || x(state.time) > 100
    }
    const node = $('playhead')
    if (node) {
