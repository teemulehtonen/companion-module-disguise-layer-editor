...
function browserMain(applyEditPatch, discreteSegments, selectionScroll, timecode, duplicateMarkerKeys, createUiGeometry, createPresentationQueue, acceptPlaybackSample, extrapolatedPlayback) {
  ...
  function snapPoints(excludeLayer, parameter, keyTime, annotation, options=snapOptions) {
    ...
    const movingGroup = parameter===undefined && state.layers?.find(l=>l.uid===excludeLayer)?.group
    if(movingGroup) for(const layer of state.layers || []) {
      let parent=layer.parent
      while(parent) {if(parent===excludeLayer){excluded.add(layer.uid);break} parent=state.layers.find(l=>l.uid===parent)?.parent}
    }
  ...
  async function sendEdit(action, options = {}) {
    ...
    try {
      ...
      if(result.groupMove) for(const item of result.groupMove.members) {
        const current=state.layers.find(l=>l.uid===item.uid)
  ...
  function mouseDial(node, layer, action, parameter, keyTime) {
    if (!node || (layer.group && action!=='field')) return
    function preview(g) {
      g.previewFrame=0
    ...
    function previewLayer(g,destination) {
      if(g.layerGroup) {
        const delta=(destination-g.layerStart)/span*g.width
        for(const lane of g.layerLanes) lane.style.translate=delta+'px 0'
        return
      }
    ...
    node.addEventListener('pointerdown', event => {
      ...
      const movingIds=new Set([layer.uid])
      if(layer.group) for(const item of state.layers) {
        let parent=item.parent
        while(parent){if(parent===layer.uid){movingIds.add(item.uid);break}parent=state.layers.find(l=>l.uid===parent)?.parent}
      }
    ...
    const move = async event => {
      ...
      if (!g.ready) {
        g.preparing = true
        try {
          const ok = layer.group ? await releaseViewerMode() : g.keyMode ? await chooseKey(layer,parameter,g.originTime)
          ...
          if (!layer.group && !g.keyMode && state.editor.layerEdit !== 'edit' && !await sendEdit('layer_edit')) { mouseGesture = null; return }
      ...
      const target = snappedTime(g.originTime+(g.lastX-g.originX)*span/g.width,g.points,event.altKey,g.width,!layer.group && !g.keyMode && action === 'field' ? g.layerEnd-g.layerStart : 0)
      ...
      try {
        ...
        ok = layer.group
          ? await sendEdit('group_move',{trackUid:state.trackUid,layerUid:layer.uid,members:state.layers.filter(l=>g.movingIds.has(l.uid)).map(l=>({uid:l.uid,start:l.start,end:l.end})),targetTime:target.time,snap:target.snap,snapGrid:target.snapGrid})
          : numeric
  ...
  function waveform(wave, label, uid, stickyTop) {
    ...
    const audioStart = owner ? owner.start : state.trackAudio?.start
    if (!Number.isFinite(audioStart)) {
      r.lane.append(el('span', 'wave-status', 'TRACK AUDIO POSITION UNAVAILABLE'))
      return
    }
    const svg = document.createElementNS('http://www.w3.org/2000/svg', 'svg')
    svg.setAttribute('viewBox', '0 0 1000 54')
  ...
  function draw() {
    ...
    const audioToggle = el('button', 'track-audio-toggle')
    audioToggle.title = trackAudioVisible ? 'HIDE TRACK AUDIO' : 'SHOW TRACK AUDIO'
    audioToggle.setAttribute('aria-label', 'TRACK AUDIO')
    audioToggle.setAttribute('aria-pressed', String(trackAudioVisible))
    const audioIcon = document.createElementNS('http://www.w3.org/2000/svg', 'svg')
    audioIcon.setAttribute('viewBox', '0 0 22 20')
    audioIcon.setAttribute('aria-hidden', 'true')
    const audioPath = document.createElementNS(audioIcon.namespaceURI, 'path')
    audioPath.setAttribute('d', 'M3 8v4M7 5v10M11 2v16M15 5v10M19 8v4')
    audioIcon.append(audioPath)
    audioToggle.append(audioIcon)
    audioToggle.onclick = () => { trackAudioVisible = !trackAudioVisible; draw() }
    displayControls.append(audioToggle)
    ...
    if (state.trackAudio && trackAudioVisible) waveform(state.trackWaveform, 'TRACK AUDIO', undefined, headerTop)
    ...
    for (const layer of state.layers) {
      ...
      r.root.classList.toggle('group-layer',Boolean(layer.group))
      ...
      if (!layer.group) r.side.append(image(resource))
      r.side.append(layerLabel, info)
  ...
  function showSynchronizing(incoming) {
    if (!incoming.synchronizing) return false
    latestLive = null
    rendered = ''; lastFullRead = 0
    presentation.clear()
    if (state) {
      state.editEnabled = false; state.seekEnabled = false; state.selectionEnabled = false
      updateEditorControls()
    }
    $('status').textContent = incoming.connected === false ? 'CONNECTION LOST' : 'SYNCHRONISING'
    $('status').className = incoming.connected === false ? 'error' : ''
    return true
  }
  async function pollLive() {
    try {
      if (!document.hidden && state) {
        ...
        if (showSynchronizing(incoming)) return
        if ((incoming.editRevision || 0) < confirmedRevision) return
        if (readRevision !== editRevision || interactionBusy) return
        latestLive = incoming
        if (JSON.stringify(incoming.editor?.gridSteps) !== JSON.stringify(state.editor?.gridSteps) || incoming.transportUid !== state.transportUid || incoming.trackUid !== state.trackUid || incoming.focusUid !== state.focusUid || (incoming.contentRevision !== state.contentRevision && !incoming.editPatch && performance.now()>=liveGeometryUntil) || incoming.tempoKey !== state.tempoKey) {
          lastFullRead = 0
          rendered = ''
        }
        if (incoming.transportUid === state.transportUid && incoming.trackUid === state.trackUid) {
  ...
  function mergeLiveClock(snapshot, live, minimumRevision) {
    ...
    if(!live || (live.editRevision || 0)<Math.max(minimumRevision,snapshot.editRevision || 0) ||
      live.transportUid!==snapshot.transportUid || live.trackUid!==snapshot.trackUid || live.focusUid!==snapshot.focusUid)return
    ...
    try {
      if (!document.hidden && !mouseGesture && !layerMarquee && !editPending && !interactionBusy && performance.now()>=liveGeometryUntil) {
        ...
        if (full) {
          ...
          if (compatibleLive && (compatibleLive.transportUid !== incoming.transportUid || compatibleLive.trackUid !== incoming.trackUid || compatibleLive.focusUid !== incoming.focusUid)) {
            lastFullRead = 0
            rendered = ''
            return
          }
