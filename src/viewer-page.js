...
function browserMain(applyEditPatch, discreteSegments, selectionScroll, timecode, duplicateMarkerKeys) {
  ...
  function keyWheel(event,node,layer,parameter) {
    const e=state?.editor
    ...
    if(event.ctrlKey || event.shiftKey || event.altKey || event.metaKey || !event.deltaY ||
      ...
      e.layerUid!==layer.uid || e.parameter!==parameter || Math.abs(e.moveKey.time-Number(node.dataset.keyTime))>1e-6 ||
      !field || field.resource || field.discrete || field.choices?.length || field.choiceError || mouseGesture) return
    event.preventDefault();event.stopPropagation()
    const identity=wheelIdentity(), direction=event.deltaY<0?1:-1
    if(wheelEdit) {
      if(wheelEdit.identity===identity) wheelEdit.direction=direction
      return
    }
    if(editPending || interactionBusy) return
    const job={identity,direction};wheelEdit=job
    void interact(async()=>{
      try {
        while(job.direction && wheelIdentity()===identity) {
          const next=job.direction;job.direction=0
          if(!await sendEdit('value',{direction:next}))break
        }
      } finally {wheelEdit=null}
    })
  }
  function pointClick(node, layer, point, parameter, keyTime) {
    ...
    if (point === 'key') {
      ...
      node.addEventListener('wheel',event=>keyWheel(event,node,layer,parameter),{passive:false})
    }
