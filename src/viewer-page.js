...
function browserMain(applyEditPatch, discreteSegments, selectionScroll, timecode, duplicateMarkerKeys, createUiGeometry) {
  ...
  function groupIcon() {
    const box=el('span','group-icon')
    box.title='GROUP';box.setAttribute('aria-label','GROUP')
    const svg=document.createElementNS('http://www.w3.org/2000/svg','svg')
    svg.setAttribute('viewBox','0 0 24 24')
    const path=document.createElementNS(svg.namespaceURI,'path')
    path.setAttribute('d','M3 7V4h7l2 3h9v13H3z M7 11h10 M7 15h10')
    svg.append(path);box.append(svg);return box
  }
  ...
  function draw() {
    ...
    for (const layer of state.layers) {
      ...
      r.side.append(layer.group ? groupIcon() : image(resource), layerLabel, info)
      ...
      if (right >= left) {
        ...
        const thumb = layer.group ? groupIcon() : image(resource)
...
.group-layer .label{background:#24213a}.group-layer .clip{background:#49416699;border-color:#8e80ba;color:#e2dafa}.group-icon{display:inline-flex;width:34px;height:28px;align-items:center;justify-content:center;flex-shrink:0;color:#b8a5e3}.group-icon svg{width:24px;height:24px;fill:none;stroke:currentColor;stroke-width:1.5;stroke-linejoin:round}.group-layer .layer-edge{border-color:#8e80ba;cursor:default}.key-point.selected-keyframe,.curve-key.selected-keyframe{scale:1.4;z-index:4}.timeline-target,.clip{touch-action:none}.key-edit-menu[hidden]{display:none}.snap-option{display:flex;align-items:center;gap:7px;padding:6px 8px;font-size:10px;white-space:nowrap;cursor:pointer}.snap-guide{position:absolute;top:0;bottom:0;width:2px;background:#ffc580;box-shadow:0 0 5px #ffc58088;z-index:8;pointer-events:none}.key-edit-menu input{background:#101a20;color:#eef4f6;border:1px solid #44616f;border-radius:3px;padding:5px}.key-edit-menu strong{font-size:11px}
