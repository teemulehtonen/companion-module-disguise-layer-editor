...
if ($version -notmatch '^0\.1\.0-beta\.\d+ { throw 'Update the release name and manifest together before creating a different release.' }
...
$sourceFiles = @(
    ...
    'docs/TIMELINE-VIEWER.md','docs/VIEWER-TEST-REPORT.md','docs/ARCHITECTURE.md','docs/DEVELOPER-MANUAL.md','docs/TRACK-1-TESTS.md','docs/TRACK-6-TESTS.md','docs/GITHUB-SETUP.md','docs/BUILD.md'
)
...
) { throw 'Update the release name and manifest together before creating a different release.' }
$releaseName = '0.1.beta'
$releaseRoot = Join-Path $projectRoot 'releases'
$destination = Join-Path $releaseRoot $releaseName
$staging = Join-Path $projectRoot '.tools/release-staging/0.1.beta'
...
$sourceFiles = @(
    ...
    'scripts/release.ps1',
    'scripts/build-page.cjs','scripts/verify-package.mjs','templates/button-style.json',
    'docs/ARCHITECTURE.md','docs/DEVELOPER-MANUAL.md','docs/TRACK-1-TESTS.md','docs/TRACK-6-TESTS.md','docs/GITHUB-SETUP.md','docs/BUILD.md'
)
foreach ($folder in @('src','test','companion')) {
    $sourceFiles += Get-ChildItem -LiteralPath (Join-Path $projectRoot $folder) -File -Recurse | ForEach-Object { $_.FullName.Substring($projectRoot.Length + 1) }
}
