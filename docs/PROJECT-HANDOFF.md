...
### Fourth CC1 LCD context repair
The fourth LCD uses pad_down/pad_up slot 3 again, so resource selection and parameter-list selection share the normal context behavior. Normal LINK TIME mode displays master_transport and transport_master_level%; other contexts display pad_3, pad_image_3 and pad_kind_3. Fader information is hidden during lists and confirmation screens. The clean page template now carries this layout. scripts/cc1-context-button.cjs builds targeted repairs for customised or plain LINK TIME buttons; cc1-context-button.test.js is included in the companion regression group. Installed binding/style readback passed without pressing buttons or sending Designer mutations. All 349 offline tests and package smoke passed; physical Yamaha rendering remains untested.

## OSC fader display names — beta.79
Connection fields oscFaderName1–8 feed masterTargetSlots names, trimmed with OSC FADER N defaults. Existing master_transport_9–16 and master_transport variables update selector and current-fader displays without changing OSC IDs or addresses. Config persistence retains the names alongside remembered levels. Validation: 351 offline tests and package smoke passed, including eight-field/default coverage, label publication, persistence, selection/value stability and unchanged OSC payload routing. Installed beta.79 enabled with unchanged existing configuration and empty last_error; read-only watchEdit confirmed all eight new fields. No live OSC or native Designer mutation test performed. No page layout changes were needed for this feature.

### Fourth LCD clean visual rebuild
Rebuilt the installed fourth LCD's style from its adjacent context button: identical canvas decoration, typography, padding, image geometry and cyan accent. Removed legacy text layers and the overriding LINK TIME colour feedback; press/release actions remained unchanged. A single standard text layer shows the selected fader name plus percentage in normal mode, and context labels otherwise. The clean page template uses the same construction via cleanContextStyle. All 352 offline tests and package smoke passed. Installed readback confirmed exactly six expected layers, unchanged actions and untouched neighbour. Physical Yamaha rendering was not inspected.

### Fourth LCD NaN correction
Companion's expression addition produced NaN for the combined fader name/value text. Replaced expression text with literal variable interpolation and mutually exclusive normal/context text layers that share identical typography and geometry. Regression now checks literal text mode and mode visibility rather than using JavaScript addition as a surrogate for Companion expressions. All 352 offline tests and package smoke passed. Installed readback confirms both literal text bindings and unchanged actions; hardware rendering not inspected.

## Finer default encoder sensitivity — beta.80
Operator reported abrupt edits across parameters. Reduced automatic range fractions from 10%/1%/0.1% to 1%/0.1%/0.01%, identically in JavaScript demo and generated native Python. Float fallback without usable min/max is also tenfold smaller; explicit action step overrides are unchanged and integer resolution remains at least one. Existing automatic decimal precision displays the finer steps. Regression covers signed, offset, tiny ranges, constants and keys, clamps, enum/integer behavior, Python parity and explicit overrides. All 353 offline tests and package smoke passed. Installed beta.80 read-only verification confirmed enabled, unchanged settings and empty last_error. No native Designer mutation or physical encoder test performed; operator should assess the new sensitivity on Yamaha.
